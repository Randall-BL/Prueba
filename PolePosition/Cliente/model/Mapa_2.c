...
int height_2 = 1020;
int roadW_2 = 2000;
int segL_2 = 200; // segment length
float camD_2 = 0.84; // camera depth
...
void project_2(Line_2* line, int camX, int camY, int camZ) {
    line->scale = camD_2 / (line->z - camZ);
    line->X = (1 + line->scale * (line->x - camX)) * width_2 / 2;
    line->Y = (1 - line->scale * (0 - camY)) * height_2 / 2;
    line->W = line->scale * roadW_2 * width_2 / 2;
}
...
void drawSprite_2(Line_2* line, sfRenderWindow* app) {
    ...
    float destX = line->X + line->scale * line->spriteX * width_2 / 2;
...
void Ejecutar_Mapa2(sfRenderWindow* ventana_juego) {
    ...
    if (sfKeyboard_isKeyPressed(sfKeyUp)) speed = 200;
    if (sfKeyboard_isKeyPressed(sfKeyDown)) speed = -200;
    ...
    pos += speed;
    while (pos >= N * segL_2) pos -= N * segL_2;
    while (pos < 0) pos += N * segL_2;

    int startPos = pos / segL_2;
    int camH = H;
    ...
    for (int n = startPos; n < startPos + 300; n++) {
        Line_2* l = &lines[n % N];
        project_2(l, playerX * roadW_2 - x, camH, startPos * segL_2 - (n >= N ? N * segL_2 : 0));
        x += dx;
        dx += l->curve;
        ...
        Line_2* p = &lines[(n - 1) % N];
        drawQuad_2(ventana_juego, grass, 0, p->Y, width_2, 0, l->Y, width_2);
        drawQuad_2(ventana_juego, rumble, p->X, p->Y, p->W * 1.2, l->X, l->Y, l->W * 1.2);
        drawQuad_2(ventana_juego, road, p->X, p->Y, p->W, l->X, l->Y, l->W);
    }

    // Dibujar los objetos únicos (sprites)
    drawSprite_2(&lines[spriteSegment1], ventana_juego);
    drawSprite_2(&lines[spriteSegment2], ventana_juego);
    drawSprite_2(&lines[spriteSegment3], ventana_juego);
